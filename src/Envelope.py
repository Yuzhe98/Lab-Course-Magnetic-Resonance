...
# This script was initially generated by ChatGPT.
##########################################
...
ureg = UnitRegistry()
# Define units
ureg.define("parsec = 30856775814913673 * meter = pc")  # Parsec
...
class PhysicalQuantity:
    ...
    # def __str__(self):
    #     """
    #     Default human-readable string, with 2 decimal places.
    #     """
    #     return self.to_string(precision=2)

    def to_string(self, precision=2):
        """
        String with user-defined precision.
        ...
        """
        val = round(self.value, precision)
        return f"{val:.{precision}f} {self.unit}"

    def __format__(self, format_spec):
        """
        Support commands like format(value, ".3f") or f"{value:.3f}"
        """
        formatted_val = format(self.value, format_spec)
        return f"{formatted_val} {self.unit}"
    ...
    def __abs__(self):
        """
        Return the absolute value of a physical quantity.
        """
        result = (self.quantity**2) ** (1 / 2)
        return PhysicalQuantity(result.magnitude, str(result.units))
...
if __name__ == "__main__":
    ...
    parsecFromAU = au * 648000.0 / (np.pi)
...
# mole to number by Avogadro's number
N_A = PhysicalQuantity(6e23, "mole**(-1)")

# Constants

# Speed of light in vacuum
c = PhysicalQuantity(1, "c")  # 299792458 [m/s]

# Gravitational constant
G = PhysicalQuantity(
    6.67430e-11, "m**3 * kg ** (-1) s ** (-2)"
)  # 6.67430(15)×10^−11 [m³/kg/s²]
